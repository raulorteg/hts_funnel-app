...
class MolGraph(object):
    ...
    def label_tree(self):
        def dfs(order, pa, prev_sib, x, fa):
            pa[x] = fa
            sorted_child = sorted(
                [y for y in self.mol_tree[x] if y != fa]
        ...
        for i, cls in enumerate(self.clusters):
            inter_atoms = (
                set(cls) & set(self.clusters[pa[i]]) if pa[i] >= 0 else set([0])
            )
            ...
            if (
                pa[i] >= 0 and len(self.clusters[pa[i]]) > 2
            ):  # uncertainty occurs in assembly
                hist = [a for c in prev_sib[i] for a in self.clusters[c]]
                pa_cls = self.clusters[pa[i]]
                tree.nodes[i]["assm_cands"] = get_assm_cands(
                    mol, hist, inter_label, pa_cls, len(inter_atoms)
                )
                    ...
                    for ch_atom in self.mol_graph[fa_atom]:
                        if ch_atom in diff:
                            ...
                            if (
                                ...
                                self.mol_graph[ch_atom][fa_atom]["label"] = (
                                    label,
                                    child_order,
                                )
        return order
    ...
    def tensorize(mol_batch, vocab, avocab):
        ...
        graph_tensors, graph_batchG = MolGraph.tensorize_graph(
            [x.mol_graph for x in mol_batch], avocab
        )
        ...
        for v, attr in tree_batchG.nodes(data=True):
            ...
            tree_batchG.nodes[v]["inter_label"] = inter_label = [
            ...
            tree_batchG.nodes[v]["assm_cands"] = [
                add(x, offset) for x in attr["assm_cands"]
            ]
