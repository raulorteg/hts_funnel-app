...
from funnel.generator.inc_graph import IncGraph, IncTree
...
class MPNEncoder(nn.Module):
    ...
    def forward(self, fnode, fmess, agraph, bgraph):
        ...
        return (
            node_hiddens * mask,
            h,
        )  # return only the hidden state (different from IncMPNEncoder in LSTM case)


class HierMPNEncoder(nn.Module):
    """
    Hierarchical Message Passing Network Decoder, torch.nn.Module
    implementation of the decoder for both the VAE and CVAE (since the encoder arquiecture is the same, is the decoder that changes)
    """
    ...
    def __init__(
        ...
        self.E_c = nn.Sequential(
            nn.Embedding(vocab.size()[0], embed_size), nn.Dropout(dropout)
        )
        self.E_i = nn.Sequential(
            nn.Embedding(vocab.size()[1], embed_size), nn.Dropout(dropout)
        )
        ...
        self.tree_encoder = MPNEncoder(
            rnn_type=rnn_type,
            input_size=hidden_size + MolGraph.MAX_POS,
            node_fdim=hidden_size,
            hidden_size=hidden_size,
            depth=depthT,
            dropout=dropout,
        )
        self.inter_encoder = MPNEncoder(
            rnn_type=rnn_type,
            input_size=hidden_size + MolGraph.MAX_POS,
            node_fdim=hidden_size,
            hidden_size=hidden_size,
        ...
        self.graph_encoder = MPNEncoder(
            rnn_type=rnn_type,
            input_size=atom_size + bond_size,
            node_fdim=atom_size,
            hidden_size=hidden_size,
            depth=depthG,
            dropout=dropout,
...
class IncMPNEncoder(MPNEncoder):
    def __init__(
        ...
        super(IncMPNEncoder, self).__init__(
            rnn_type, input_size, node_fdim, hidden_size, depth, dropout
        )
...
class IncHierMPNEncoder(HierMPNEncoder):
    def __init__(
        ...
        super(IncHierMPNEncoder, self).__init__(
            device,
            vocab,
            avocab,
            rnn_type,
        ...
        self.tree_encoder = IncMPNEncoder(
            rnn_type=rnn_type,
            input_size=hidden_size + MolGraph.MAX_POS,
            node_fdim=hidden_size,
            hidden_size=hidden_size,
        ...
        self.inter_encoder = IncMPNEncoder(
            rnn_type=rnn_type,
            input_size=hidden_size + MolGraph.MAX_POS,
            node_fdim=hidden_size,
            hidden_size=hidden_size,
        ...
        self.graph_encoder = IncMPNEncoder(
            rnn_type=rnn_type,
            input_size=self.atom_size + self.bond_size,
    ...
    def get_sub_tensor(self, tensors, subset):
        ...
        agraph, bgraph = agraph.index_select(0, subnode), bgraph.index_select(
            0, submess
        )
    ...
    def embed_sub_tree(self, tree_tensors, hinput, subtree, is_inter_layer):
        ...
        fnode, fmess, agraph, bgraph, cgraph, _ = self.get_sub_tensor(
            tree_tensors, subtree
        )
    ...
    def forward(
        ...
        if len(subgraph[0]) + len(subgraph[1]) > 0:
            sub_graph_tensors = self.get_sub_tensor(graph_tensors, subgraph)[
                :-1
            ]  # graph tensor is already embedded
            hgraph.node, hgraph.mess = self.graph_encoder(
                sub_graph_tensors, hgraph.mess, num_graph_nodes, subgraph
            )

        if len(subtree[0]) + len(subtree[1]) > 0:
            sub_inter_tensors = self.embed_sub_tree(
                inter_tensors, hgraph.node, subtree, is_inter_layer=True
            )
            hinter.node, hinter.mess = self.inter_encoder(
                sub_inter_tensors, hinter.mess, num_tree_nodes, subtree
            )

            sub_tree_tensors = self.embed_sub_tree(
                tree_tensors, hinter.node, subtree, is_inter_layer=False
            )
            htree.node, htree.mess = self.tree_encoder(
                sub_tree_tensors, htree.mess, num_tree_nodes, subtree
            )
...
class CondHierMPNDecoder(nn.Module):
    """
    Condtional Hierarchical Message Passing Network Decoder, torch.nn.Module
    implementation of the decoder for the Conditional Hierarchical VAE (CVAE)
    """
    ...
    def __init__(
        ...
        super(CondHierMPNDecoder, self).__init__()
        ...
        self.W_root = nn.Linear(latent_size + 1, hidden_size)
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders, cond):
        ...
        # here append the new concatenated dimension to make conditional
        cond = torch.Tensor(cond)
        cond = torch.unsqueeze(cond, 1).to(self.device)
        batch_size = src_root_vecs.shape[0]
        src_root_vecs_cond = torch.cat((src_root_vecs, cond), dim=1)

        # init_vecs = src_root_vecs if self.latent_size == self.hidden_size else self.W_root(src_root_vecs)
        init_vecs = self.W_root(src_root_vecs_cond)
        ...
        for t in range(maxt):
            ...
            subtree = htree.emask.new_tensor(subtree[0]), htree.emask.new_tensor(
                subtree[1]
            )
        ...
        topo_vecs, batch_idx, topo_labels = zip_tensors(
            all_topo_preds, device=self.device
        )
        ...
        cls_vecs, batch_idx, cls_labs, icls_labs = zip_tensors(
            all_cls_preds, device=self.device
        )
        ...
        if len(all_assm_preds) > 0:
            assm_vecs, batch_idx, assm_labels = zip_tensors(
                all_assm_preds, device=self.device
            )
    ...
    def decode(
        ...
        tree_batch = IncTree(
            device=self.device, batch_size=batch_size, node_fdim=2, edge_fdim=3
        )
        # device: torch.device, avocab, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=300, max_nb=10):
        ...
        for t in range(max_decode_step):
            ...
            subgraph = batch_idx.new_tensor(
                tree_batch.get_cluster_nodes(cur_tree_nodes)
            ...
            for i, bid in enumerate(expand_list):
                ...
                for kk in cls_beam:  # try until one is chemically valid
                    ...
                    smiles, ismiles = self.vocab.get_smiles(
                        clab
