...
import torch
from funnel.generator.chemutils import *
...
class IncBase(object):
    def __init__(
        ...
        self.fnode = (
            torch.zeros(max_nodes * batch_size, node_fdim).long().to(self.device)
        )
    ...
    def add_edge(self, i, j, feature=None):
        ...
        in_edges = [
            self.edge_dict[(k, i)] for k in self.graph.predecessors(i) if k != j
        ]
...
class IncTree(IncBase):
    def __init__(
        ...
        super(IncTree, self).__init__(
            device, batch_size, node_fdim, edge_fdim, max_nodes, max_edges, max_nb
        )
    ...
    def get_cluster_nodes(self, node_list):
        return [
            c for node_idx in node_list for c in self.graph.nodes[node_idx]["cluster"]
        ]
    ...
    def get_cluster_edges(self, node_list):
        return [
            ...
            for e in self.graph.nodes[node_idx]["cluster_edges"]
        ]
...
class IncGraph(IncBase):
    def __init__(
        ...
        self.mol.AddAtom(
            Chem.Atom("C")
        )  # make sure node is 1 index, consistent to self.graph
    ...
    def add_mol(self, batch_idx, smiles, inter_label, nth_child):
        ...
        for (
            ...
            emol.GetAtoms()
        ):  # atoms must be inserted in order given by emol.GetAtoms() (for rings assembly)
            if atom.GetIdx() in atom_map:
                ...
                assert idx == self.add_node(
                    self.get_atom_feature(new_atom)
        ...
        for bond in emol.GetBonds():
            ...
            if existing_bond is None:
                ...
                self.add_edge(
                    a1,
                    a2,
                    self.get_mess_feature(
                        bond.GetBeginAtom(),
                        bond_type,
                        nth_child if a2 in attached else 0,
                    ),
                ...
                self.add_edge(
                    ...
                    a1,
                    self.get_mess_feature(
                        bond.GetEndAtom(), bond_type, nth_child if a1 in attached else 0
                    ),
                )
            else:
    ...
    def try_add_mol(self, batch_idx, smiles, inter_label):
        ...
        for bond in emol.GetBonds():
            ...
            bond_type = bond.GetBondType()
            if (
                self.mol.GetBondBetweenAtoms(a1, a2) is None
            ):  # later maybe check bond type match
                self.mol.AddBond(a1, a2, bond_type)
    ...
    def get_mess_feature(self, atom, bond_type, nth_child):
        ...
        f3[nth_child] = 1
        return torch.cat([f1, f2, f3], dim=-1).to(self.device)  # .cuda()
    ...
    def get_assm_cands(self, cluster, used, smiles):
        ...
        if emol.GetNumAtoms() == 1:
            ...
            attach_points = [
                atom.GetIdx() for atom in emol.GetAtoms() if atom.GetAtomMapNum() > 0
            ]
        ...
        if inter_size == 1:
            ...
            anchors = [
                a for a in attach_points if is_anchor(emol.GetAtomWithIdx(a), [0])
            ]  # all attach points are labeled with 1
            attach_points = [a for a in attach_points if a not in anchors]
            attach_points = (
                [anchors[0]] + attach_points + [anchors[1]]
        ...
        if inter_size == 1:
            ...
            cands = [
                cluster2[i : i + inter_size] for i in range(len(cluster))
            ]  # not pairs if inter_size >= 3
            cands = [
                c
                for c in cands
                if (c[0], c[-1]) not in used
                and bond_match(
                    self.mol, c[0], c[-1], emol, attach_points[0], attach_points[-1]
                )
            ]  # weak matching
        else:
            cluster2 = cluster + cluster
